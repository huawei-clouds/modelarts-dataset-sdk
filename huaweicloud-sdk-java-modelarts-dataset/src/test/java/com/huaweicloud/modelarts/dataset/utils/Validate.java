...
public class Validate
{
    ...
    public static void validateClassificationDetection(Dataset dataset)
    {
        ...
        for (int i = 0; i < sampleList.size(); i++)
        {
            ...
            Assert.assertTrue(
                sample.getUsage().equalsIgnoreCase("TRAIN") || sample.getUsage().equalsIgnoreCase("INFERENCE"));
            List<Annotation> annotationList = sample.getAnnotations();
            ...
            if (null != annotationList)
            {
                for (int j = 0; j < annotationList.size(); j++)
                {
                    ...
                    assertEquals(annotation.getAnnotatedBy(), "human");
                    Assert.assertTrue(null == annotation.getAnnotationFormat() ||
                        annotation.getAnnotationFormat().equalsIgnoreCase("PASCAL VOC"));
                }
            }
        }
    }
